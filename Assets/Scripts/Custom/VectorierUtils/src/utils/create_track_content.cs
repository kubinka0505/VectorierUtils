...
public class DZMake : MonoBehaviour {
	...
	[MenuItem("Vectorier/⚙ Utils/🗃️ Advanced level build + Run game (Steam)")]
	...
	void Awake() {
		...
		if (buildMap != null) {
			musicFilePath = Path.Combine(gamePath, buildMap.levelMusic + ".mp3");

			// asserts background is in PNG format
			backgroundImagePath = Path.Combine(directoryAssetsTextures, buildMap.customBackground + ".png");
		}
	}
